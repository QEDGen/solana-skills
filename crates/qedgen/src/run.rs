...
/// Paths a generator default used to write to, paired with what replaced
/// each. Swept once per `codegen` run (#369).
///
/// When a default moves, the file at the OLD path is not compiled (no `mod`
/// hook, wrong directory), not removed, and still drift-compared — so
/// `check --regen-drift --write` keeps refreshing a file nothing builds. It
/// looks maintained and is dead. Three bundled examples carried orphans at
/// two of these paths for many releases.
///
/// Relative to the spec's directory, like every other codegen output.
const RETIRED_OUTPUT_PATHS: &[(&str, &str)] = &[
    // Unit tests moved out of `src/` in v2.47: a `src/` location needs a
    // `mod` hook the scaffold never emits, so cargo never built it.
    ("src/tests.rs", "programs/tests/unit.rs"),
    ("programs/src/tests.rs", "programs/tests/unit.rs"),
    // Integration tests moved under the program package so the Parallax
    // dev-dependencies in `programs/Cargo.toml` govern them.
    (
...
/// Does this file belong to the generator? The banner is the only
/// in-band marker, so a file without one is treated as the user's.
fn is_generator_owned(body: &str) -> bool {
    body.contains("GENERATED BY QEDGEN")
        || body.contains("QuasarSVM integration test scaffold")
        || body.contains("Parallax integration test scaffold")
}

/// Delete generator-owned files stranded at retired default paths (#369).
///
/// `active_outputs` are the paths THIS run writes. A user who explicitly
/// passes a retired path (`--test-output src/tests.rs`) has chosen it, and
/// sweeping it would delete the artifact they asked for.
fn sweep_retired_output_paths(root: &Path, active_outputs: &[&Path]) -> Result<()> {
    for (retired, replacement) in RETIRED_OUTPUT_PATHS {
        let path: PathBuf = root.join(retired).components().collect();
        if !path.is_file() || active_outputs.iter().any(|active| *active == path) {
            continue;
        }
        let body = std::fs::read_to_string(&path)?;
        if is_generator_owned(&body) {
            std::fs::remove_file(&path)?;
            eprintln!(
                "note: removed {} — a generated artifact at a retired output path. \
                 It moved to {}, and nothing compiled it where it was.",
                path.display(),
                replacement
            );
        } else {
            eprintln!(
                "warning: {} sits at a retired generator output path (now {}), but has \
                 no QEDGEN banner, so it looks user-authored — leaving it in place.",
                path.display(),
                replacement
            );
        }
    ...
    match cmd {
        ...
        Commands::Discharge {
            ...
            if transition {
                ...
                descriptor::run_discharge(&parsed, &request)?;
            }
        }
        ...
        Commands::Codegen {
            ...
            // Before generating anything: drop generator-owned files left at
            // paths a default used to write to (#369). Ahead of generation so
            // an explicitly-requested retired path is rewritten rather than
            // swept, and so the note reads before the "Generated N files"
            // line it explains.
            sweep_retired_output_paths(
                &spec_dir,
                &[
                    &output_dir,
                    &kani_output,
...
mod integration_cleanup_tests {
    ...
    #[test]
    fn removes_generator_owned_parallax_quasar_client_file_on_anchor_target() {
        ...
        assert!(
            !path.exists(),
            "generator-owned Parallax/Quasar-client scaffold must be removed for a non-Quasar target"
        );
    }
    ...
    /// #369 — a generated artifact stranded at a retired default path is
    /// swept; a user-authored file at the same path is not.
    #[test]
    fn retired_output_paths_are_swept_but_user_files_survive() {
        let temp = tempfile::tempdir().unwrap();
        let root = temp.path();
        std::fs::create_dir_all(root.join("src")).unwrap();
        std::fs::create_dir_all(root.join("programs/src")).unwrap();

        let generated = root.join("src/tests.rs");
        std::fs::write(&generated, "// ---- GENERATED BY QEDGEN ----\nfn t() {}\n").unwrap();
        let handwritten = root.join("programs/src/tests.rs");
        std::fs::write(&handwritten, "fn mine() {}\n").unwrap();

        sweep_retired_output_paths(root, &[]).unwrap();

        assert!(!generated.exists(), "banner file at a retired path must go");
        assert!(
            handwritten.exists(),
            "a file with no banner is the user's, not ours"
        );
    }

    /// A retired path the user explicitly asked THIS run to write is not
    /// swept — otherwise codegen would delete the artifact it was told to
    /// produce.
    #[test]
    fn an_explicitly_requested_retired_path_is_left_alone() {
        let temp = tempfile::tempdir().unwrap();
        let root = temp.path();
        std::fs::create_dir_all(root.join("src")).unwrap();
        let chosen = root.join("src/tests.rs");
        std::fs::write(&chosen, "// ---- GENERATED BY QEDGEN ----\nfn t() {}\n").unwrap();

        sweep_retired_output_paths(root, &[&chosen]).unwrap();

        assert!(
            chosen.exists(),
            "the requested output must survive its own run"
        );
    }
}
