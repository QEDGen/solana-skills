...
| `--integration` | bool | false | Generate Parallax/LiteSVM integration tests. Anchor and Quasar (#366); skipped with a note on `pinocchio`, whose leading-discriminant-byte dispatch needs its own builder. Quasar builds instructions through the generated `program::client`; Anchor has no generated client, so the scaffold spells out the ABI (`sha256("global:<handler>")[..8]` + declared account metas + Borsh args). Run it AFTER the program scaffold: the Parallax dev-dependency upsert needs a `Cargo.toml` to write into. |
