...
| `crucible` on PATH | fast auto-reproducer lane off | Lose the ~5–10 min zero-authoring path (`qedgen probe --fuzz`) — but scope it honestly: in spec-less brownfield mode Crucible runs a per-action protocol-invariant suite that fires on failure modes observable as a mechanical state diff — wallet-lamport inflation (any fuzzer-controlled account, signer or not, gaining lamports), total-lamport conservation, ownership takeover, discriminator/type change, close-scrub integrity, rent-exemption loss, realloc data leak, and SPL token-balance conservation. In-program faults (overflow, `checked_*` DoS, div-by-zero, `unwrap`, `require!`) surface as tx-errors it can't see, and semantic/DeFi bugs need spec context. Those ALWAYS need agent-authored Mollusk repros regardless of Crucible. Treat Crucible as a fast win for the state-diff-observable shapes, not a general crash lane. |
