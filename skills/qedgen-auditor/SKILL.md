...
| `crucible` on PATH | fast auto-reproducer lane off | Lose the ~5–10 min zero-authoring path (`qedgen probe --fuzz`) — but scope it honestly: in spec-less brownfield mode Crucible's live detectors are two per-action protocol guards — signer-lamport inflation (**drain-shaped** findings: funds to a tracked account) and ownership takeover (a tracked account's `owner` flipping across a call). In-program faults (overflow, `checked_*` DoS, div-by-zero, `unwrap`, `require!`) surface as tx-errors it can't see. Arithmetic and other non-drain/non-takeover findings ALWAYS need agent-authored Mollusk repros regardless of Crucible. Treat Crucible as a fast win for drain/takeover shapes, not a general crash lane. |
