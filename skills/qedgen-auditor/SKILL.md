...
| `crucible` on PATH | fast auto-reproducer lane off | Lose the ~5–10 min zero-authoring path (`qedgen probe --fuzz`) — but scope it honestly: in spec-less brownfield mode Crucible's only live detector is the signer-lamport-inflation guard, so it fires on **drain-shaped** findings (funds flowing to a tracked signer) and nothing else. In-program faults (overflow, `checked_*` DoS, div-by-zero, `unwrap`, `require!`) surface as tx-errors it can't see. Arithmetic and non-drain findings ALWAYS need agent-authored Mollusk repros regardless of Crucible. Treat Crucible as a fast win for drains, not a general crash lane. |
