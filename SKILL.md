...
**Types:**
- `Pubkey` — `{ c0 : Nat, c1 : Nat, c2 : Nat, c3 : Nat }` (four LE U64 chunks of a 32-byte key)
- `U64` (= Nat), `U8` (= Nat)
...
**Execution:**
- `executeFn (fetch : Nat → Option Insn) (s : State) (fuel : Nat) : State` — function-based fetch (O(1) per step). Use with match-based `progAt` functions (generated by `asm2lean`). Use `wp_exec` tactic to prove properties.
...
- `Pubkey` — 32-byte pubkey as four LE U64 chunks (`.c0`, `.c1`, `.c2`, `.c3`); same type as `QEDGen.Solana.Pubkey`
