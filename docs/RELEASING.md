...
5a. **`bash scripts/check-parallax-pin.sh`** — reports how far the pinned Parallax revision has fallen behind upstream. `parallax-svm` is unpublished, so the generated integration scaffold pins a git rev; `parallax_integration_gate` proves the scaffold compiles against that rev, but nothing else notices it aging. Being behind is **expected and exits 0** — pinning is what stops upstream churn from breaking generated code. It exits **1** only when the pinned rev no longer exists upstream (force-push/rebase), which means the dependency will not resolve. Bumping the pin means editing `PARALLAX_GIT_REV` in `crates/qedgen/src/codegen/integration_test.rs` (the single source; a unit test holds the gate fixture to it), re-running `cargo test -p qedgen-solana-skills --test parallax_integration_gate -- --ignored`, and regenerating the bundled examples.
...
9. **Doc/code drift sweep** — README, SKILL.md, CLAUDE.md, `references/`, `docs/design/`, this file, `docs/prds/RELEASE-v<version>.md`, and module `//!` docstrings all have to match shipped reality. The `check-readme-drift.sh` script only covers top-level command coverage in README; everything else needs an explicit pass. Concretely:
   ...
   - **`bash scripts/check-closed-issue-refs.sh`** (#356) — flags capability-gap lines (in `references/`, README, SKILL.md, this file, `cli.rs` help text, and the obligation reason strings) that cite a CLOSED issue. This class bit three surfaces before the script existed: the #260 caveat carried in this file for ten releases, the `verify --strict` flag row in cli.md, and the emitted `UnsupportedReason` strings (#354). Needs authenticated `gh`; a deliberate closed-issue citation takes an inline `closed-issue-ok` marker.
   - Re-verify every documented "known gaps" list against reality: run `qedgen check --coverage` on the bundled examples and compare the `unsupported` reasons against what cli.md and the release notes claim is still unsupported. The v2.48.0 cut found the `verify --strict` row listing four gaps the same release wave had closed.
