...
5a. ~~`bash scripts/check-parallax-pin.sh`~~ — **now mechanical (#371)**. `scripts/release-gate.sh` calls it, and `.github/workflows/parallax-pin.yml` runs it weekly, because its failing condition (the pinned revision vanished upstream) breaks generated scaffolds in a user's crate and has nothing to do with release timing.

    Only the **pin bump** is manual, and only when you want a newer Parallax API: edit `PARALLAX_GIT_REV` in `crates/qedgen/src/codegen/integration_test.rs` (the single source — codegen writes it into every generated `[dev-dependencies]`), re-run `cargo test -p qedgen-solana-skills --test parallax_integration_gate -- --ignored`, and regenerate the bundled examples. Being behind is expected and never blocks a release; pinning is what stops upstream churn from breaking generated code.
